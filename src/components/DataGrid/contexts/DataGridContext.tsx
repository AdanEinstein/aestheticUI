...
import { Dispatch, PropsWithChildren, ReactNode, RefObject, SetStateAction, createContext, useContext, useRef, useState } from "react"
...
export default function DataGridProvider<T extends GridValidRowModel,>({children, params}: PropsWithChildren<IDataGridProvider<T>>){
  ...
  return (
    <DataGridContext.Provider value={{ params, setComponentDelete, setComponentEdit, modalDeleteRef, modalEditRef }} >
        {children}
        <BasicModal ref={modalDeleteRef}>
          {componentDelete}
        </BasicModal>
